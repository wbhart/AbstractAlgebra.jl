...
@doc Markdown.doc"""
    power_sums_to_polynomial(P::Array{T, 1};
      parent::AbstractAlgebra.PolyRing{T}=UPolyRing(parent(P[1]))) where T <: RingElement -> PolyElem{T}
...
function power_sums_to_polynomial(P::Array{T, 1};
            parent::AbstractAlgebra.PolyRing{T}=UPolyRing(parent(P[1]))) where T <: RingElement
